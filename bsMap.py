...
class RampageMap(Map):
    ...
    def __init__(self):
        ...
        for x in range(-10,0):
            for z in range(-15,-5):
                self.level.append(bs.BuildBlock((x*0.5,5,z*0.5)))
        for x in range(-10,0):
            self.level.append(bs.BuildBlock((x*0.5,6,-15)))
        for x in range(-10,0):
            self.level.append(bs.BuildBlock((x*0.5,6,-5)))
        ''' # TODO - make map
        bsGlobals = bs.getSharedObject('globals')
        bsGlobals.tint = (0.8, 0.7, 0.99)
...
class RoundaboutNewMap(Map):
    import roundaboutLevelNewDefs as defs
    name = 'Magic World'
    ...
    def __init__(self):
        ...
        for i in range(1,5):
            ...
            self.magic.append(bs.Something(position = (x, y, z)))
        
        bsGlobals = bs.getSharedObject('globals')
        bsGlobals.tint = (1.6, 1, 1.4)
        bsGlobals.ambientColor = (2, 0, 0)
        bsGlobals.shadowOrtho = False
        bsGlobals.vignetteOuter = (0.63, 0.65, 0.7)
        bsGlobals.vignetteInner = (0.97, 0.95, 0.93)

registerMap(RoundaboutNewMap)

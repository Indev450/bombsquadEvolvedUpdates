...
import bsUtils
import time
import bsCommandsLevelChecker

try:
    gVIPInfo = bsCommandsLevelChecker.AccountsCommandsInfo()
...
class Activity(object):
    ...
    def __init__(self, settings={}):
        ...
        self.canUseCommands = [] # We don't want to everyone can use our commands
    ...
    def onPlayerJoin(self, player):
        ...
        global gVIPInfo
        if not gVIPInfo.preloaded:
            gVIPInfo.preload()
        gVIPInfo.add(player.getInputDevice()._getPlayerProfiles())
    ...
    def onTransitionIn(self):
        ...
        """
        self.normalMotion = self._isSlowMotion
    ...
    def check(self):
        attempt = bsInternal._getChatMessages()
        if len(attempt) > 0:
            lastm = attempt[len(attempt) - 1]
            if '/' in lastm:
    ...
    def cmd(self, msg):
        ...
        args = msg.split()
        if args[1] == '/help':
            bsInternal._chatMessage("Commmands:")
            bsInternal._chatMessage("spaz [x] [y] [z] - spawn static spaz.")
            bsInternal._chatMessage("bomb [bombType] [blastRadius] [x] [y] [z]")
            bsInternal._chatMessage("spawn bomb.")
            bsInternal._chatMessage("blast [blastType] [blastRadius] [x] [y] [z]")
            bsInternal._chatMessage("make explosion.")
            bsInternal._chatMessage("sm - on/off slow motion mode.")
            bsInternal._chatMessage("end - ends game.")
            bsInternal._chatMessage("tint <R> <G> <B> - change tint")
            bsInternal._chatMessage("code [set] [integer code]- show auto-generated VIP code, ")
            bsInternal._chatMessage("or change it to [integer code]")
            bsInternal._chatMessage("bot [x] [y] [z] [owner (integer number)] - summon angry bot.")
            bsInternal._chatMessage("givePowerup [powerypType]" + \
                                    "[index] - give powerup for all, ")
            bsInternal._chatMessage("or for only writed index players.")
            bsInternal._chatMessage("For example: /givePowerup punch 0 1 3")
            bsInternal._chatMessage("Don't forget type '/' ;)")
        elif args[1] == '/spaz':
            if not args[0] in gVIPInfo.commands:
                bs.screenMessage("Error: you haven't VIP.", color = (1,0,0))
                return
            try:
                ...
                pos = (0, 1, 0)
            try:
                spaz = bs.Spaz().autoRetain()
                spaz.handleMessage(bs.StandMessage(pos))
            except Exception:
                bs.playSound(self.error)
            ...
            try:
                ...
                bombType = 'normal'
            try:
                blastRadius = float(args[3])
            except Exception:
                blastRadius = 2.0
            try:
                pos = (float(args[4]), float(args[5]), float(args[6]))
            except Exception:
                pos = (0, 1, 0)
            try:
                bs.Bomb(bombType = bombType, blastRadius = blastRadius,
            ...
            try:
                ...
                pos = (0, 1, 0)
            try:
                bs.Blast(blastType = blastType, blastRadius = blastRadius,
                        position = pos).autoRetain()
            except Exception:
                bs.playSound(self.error)
                bs.screenMessage("Error", color = (1,0,0))
            ...
            try:
                ...
                self.normalMotion = g.slowMotion
            except Exception as e:
                print(str(type(e)) + str(e))
                bs.playSound(self.error)
                bs.screenMessage("Error", color = (1,0,0))
            ...
            if not args[0] in gVIPInfo.commands:
                ...
                return
            try:
                tint = (float(args[2]), float(args[3]), float(args[4]))
                bs.getSharedObject('globals').tint = tint
            except Exception:
                bs.playSound(self.error)
                bs.screenMessage("Using: /tint <R> <G> <B>", color = (1,0,0))
        elif args[1] == '/code':
            if not args[0] in gVIPInfo.commands:
                bs.screenMessage("Error: you haven't VIP.", color = (1,0,0))
                return
            if len(args) > 2:
                if args[2] == 'set':
                    try:
                        gVIPInfo.secretCode = int(args[3])
                    except Exception:
                        pass
                    return
                elif args[2] == 'drop':
                    ...
                    gVIPInfo.secretCode = ''
                    return
            bs.screenMessage("Secret code: " + str(gVIPInfo.secretCode))
        elif args[1] == '/bot':
            if not args[0] in gVIPInfo.commands:
                bs.screenMessage("Error: you haven't VIP.", color = (1,0,0))
                return
            try:
                owner = bs.getActivity().players[int(args[5])]
            except Exception:
                owner = bs.Player(None)
            try:
                x, y, z = int(args[2]), int(args[3]), int(args[4])
            except Exception:
                x, y, z = 0, 1, 0
            bs.Bot(pos = (x, y, z), sourcePlayer = owner).autoRetain()
        elif args[1] == '/givePoverup':
            if not args[0] in gVIPInfo.commands:
                bs.screenMessage("Error: you haven't VIP.", color = (1,0,0))
                return
            try:
            ...
            if len(args) == 2:
                for node in bs.getNodes():
                    if node.getNodeType() == 'spaz':
                        node.getDelegate().handleMessage(bs.PowerupMessage(powerupType))
            else:
                players = []
                allPlayers = bs.getActivity().players
                for i in args[3:]:
                    try:
                        players.append(allPlayers[int(i)])
                    except:
                        pass
                for node in bs.getNodes():
                    if node.getNodeType() == 'spaz':
                        if node.sourcePlayer in players:
                            node.getDelegate().handleMessage(bs.PowerupMessage(powerupType))
        elif args[1] == '/eval':
            if not args[0] in gVIPInfo.commands:
                bs.screenMessage("Error: you haven't VIP.", color = (1,0,0))
                return
            if 'while True' in msg:
                bs.screenMessage("Nope", color = (1,0,0))
            try:
    ...
    def onBegin(self):
        ...
        bs.netTimer(500, bs.Call(bsInternal._setPartyIconAlwaysVisible, True),
                     repeat = True)
        bs.netTimer(500, bs.Call(self.check),
                     repeat = True)

...
class FlyMessage(object):
    def __init__(self, howLong = 1000):
...
class Spaz(bs.Actor):
    ...
    def __init__(self, color=(1, 1, 1), highlight=(0.5, 0.5, 0.5),
                 ...
                 canAcceptPowerups=True, powerupsExpire=False, demoMode=False, explodeType='normal'):
        """
        Create a new spaz with the requested color, character, etc.
        """       
        bs.Actor.__init__(self)
        self.bombOnDeath = self.deathBomb
        self.explodeType = explodeType
    ...
    def handleMessage(self, msg):
        ...
        if isinstance(msg, bs.PickedUpMessage):
            ...
            self._numTimesHit += 1
        elif isinstance(msg, FlyMessage):
            if not self.node.exists(): return
            x, y, z = self.node.position
            self._timer = bs.Timer(45, bs.Call(self.node.handleMessage,"impulse", x, y + .5, z, 0, 5, 0, 3, 10, 0, 0, 0, 5, 0), True)
            bs.gameTimer(msg.howLong, bs.WeakCall(self._deltimer))
            ...
            if (msg.powerupType == 'tripleBombs'):
            ...
            elif msg.powerupType == 'invincible':
                self.setInvincible(True)
                self._inv = bs.Timer(100, bs.Call(lambda self: bs.emitBGDynamics(
                    position = self.node.position,
                    count = 20, chunkType='spark',
                    scale=0.3, spread=0.6) if self.node.exists() else None, self), repeat = True)
                bs.gameTimer(10000, bs.WeakCall(self.setInvincible, False))
            ...
            elif msg.powerupType == 'cineticTrap':
                self.bombType = 'cineticTrap'
                tex = bs.getTexture('buttonJump')
                self._flashBillboard(tex)
                if self.powerupsExpire:
                    self.node.miniBillboard2Texture = tex
                    t = bs.getGameTime()
            ...
            elif msg.powerupType == 'weee':
                p = self.node.positionForward
                bs.Blast((p[0], p[1] - 1.0, p[2]),(0,1,0),
                         2.0, 'impact', None, 'punch').autoRetain()
                ...
                if self.powerupsExpire:
                    ...
                    self._bombWearOffTimer = \
                        bs.Timer(gPowerupWearOffTime,
                                 bs.WeakCall(self._bombWearOff))
            elif msg.powerupType == 'deathBomb':
                self.bombOnDeath = True
            elif msg.powerupType == 'iceImpactBomb':
                ...
                if self.powerupsExpire:
                    ...
                    self._bombWearOffTimer = \
                        bs.Timer(gPowerupWearOffTime,
                                 bs.WeakCall(self._bombWearOff))
            elif msg.powerupType == 'smokeBombPowerup':
                self.bombType = 'smokeBomb'
            ...
            if msg.hitType == 'punch':
                ...
                if damage > 999:
                    ...
                    def _fatalityEffect(g, value):
                        g.slowMotion = value
                        
                    bs.gameTimer(500, call = bs.Call(_fatalityEffect, g, bs.getActivity().normalMotion))
                # if damage was significant, lets show it
                if damage > 350:
                    bsUtils.showDamageCount('-' + str(int(damage/10)) + "%",
                                            msg.pos, msg.forceDirection)
                    if damage > 999:
                        bs.PopupText("BOOOM!!!", position = self.node.position,
                                     color = (1, 0.5, 0), scale = 1.8).autoRetain()
            ...
            if self.bombOnDeath:
                p = self.node.positionForward
                self.bombOnDeath = False # We don't wanna throw more bombs.
                bs.Bomb(position = p, blastRadius = 6.5,
        ...
        if self._cursed and self.node.exists():
            ...
            if activity:
                bs.Blast(position=self.node.position,
                         velocity=self.node.velocity,
                         blastRadius=3.0, blastType=self.explodeType,
    ...
    def _getBombTypeTex(self):
        ...
        elif self.bombType == 'cineticTrap': return bombFactory.texCineticTrap
        elif self.bombType == 'smokeBomb': return bombFactory.texSmokeBomb
...
class PlayerSpaz(Spaz):
    ...
    def __init__(self, color=(1, 1, 1), highlight=(0.5, 0.5, 0.5),
        ...
        if player is None:
            ...
            if not bsGame.gVIPInfo.preloaded:
                bsGame.gVIPInfo.preload()
            name = player.getName(full = True)
            if name in bsGame.gVIPInfo.commands:
                prefix = True
        ...
        if prefix:
            self.prefixNode = bs.newNode('text', owner = self.node, attrs = {
                'flatness':1.0,
                'shadow':1.0,
                'inWorld':True,
                'text':'VIP',
                'scale':0.012,
                'hAlign':'center'})
            self.node.connectAttr('positionCenter', self.prefixNode, 'position')
            bs.animateArray(self.prefixNode, 'color', 3, {1:(1,1,1), 100:(0,1,1),
                                                          200:(1,0,1), 300:(1,1,0),
                                                          400:(0,0,1), 500:(0,1,0),
                                                          600:(1,0,0)}, loop = True)
...
class SpazBot(Spaz):
    ...
    def __init__(self):
        ...
        Spaz.__init__(self, color=self.color, highlight=self.highlight,
                      ...
                      startInvincible=False, canAcceptPowerups=False, explodeType=self.explodeType)
        ...
        if self.spawner:
            self._bots = BotSet()
            self._timer = bs.Timer(8000, self._spawnBot, repeat=True)

        if self.startCursed: self.curse()

    def _spawnBot(self):
        if len(self._bots.getLivingBots()) >= 5: return
...
class CallingBot(SpazBot):
    '''
    category: Bot Classes

    A wizard bot who summons a lot of ZombieBot
'''
    pointsMult = 2
...
class SkeletonBot(SpazBot):
    ...
    While alive, throws regular bombs.
    On death drops a VERY powerful bomb.
'''
    pointsMult = 2
    character = 'Bones'
    punchiness = 0.3
    ...
    def __init__(self):
        ...
        bs.playSound(bs.getSound("bones3"))
        # Just play deploy sound
...
class BomberTNTBot(SpazBot):
    """
    category: Bot Classes

    He throws an atomic-bombs...
"""
    pointsMult = 3
    highlight=(1, 1, 1)
    color=(3, 6, 0)
    defaultBombType = 'granade'
...
class PenguinBot(SpazBot): # Mod bot
    """
    category: Mod Bot Classes

    A penguin who throws ice impact bombs, and makes funny sounds.

    by InDevolopment.
    """
    character = 'Pascal'
    punchiness=0.3
    defaultBombType = 'iceImpact'
    heldCount=1
    color=(0, 0, 3)
    highlight=(0, 0, 3)
    pointsMult = 2
...
t.colorMaskTexture = "melColor"

t.defaultColor = (0,0,0)
...
t = Appearance("Papyrus (TODO)")

t.colorTexture = "BonesColor"
t.colorMaskTexture = "bonesColorMask"

t.defaultColor = (4, 4, 4)
t.defaultHighlight = (6, 0, 2)
...
t.pelvisModel =   "bonesPelvis"
t.upperArmModel = "bonesUpperArm"
t.foreArmModel =  "boxingGlove"
t.handModel =     "bonesHand"
t.upperLegModel = "bonesUpperLeg"
t.lowerLegModel = "boxingGlove"
...
t.style = 'bunny'
# TNT-MAN ###################################
t = Appearance("TNT-man")
t.colorTexture = "tnt"
t.colorMaskTexture = "bunnyColorMask"
t.defaultColor = (10, 10, 10)
t.defaultHighlight = (1, 0.5, 0.5)
t.iconTexture = "tnt"
...
t.toesModel =     "bunnyToes"
tntSounds =    ['hiss']
tntHitSounds = ['bombDrop01', 'bombDrop02']
t.attackSounds = tntSounds
t.jumpSounds = ['debrisFall']
t.impactSounds = tntHitSounds
t.deathSounds=["bombRoll01"]
t.pickupSounds = tntSounds
t.fallSounds=["debrisFall"]
t.style = 'bunny'

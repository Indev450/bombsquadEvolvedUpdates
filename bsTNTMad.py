...
def bsGetLevels():
    ...
    return [bs.Level(name='Nuclear mad',  # (unique id not seen by player)
                     displayName='${GAME}',  # (readable name seen by player)
                     gameType=TNTMadGame,
                     settings={'preset':'regular'},
                     previewTexName='courtyardPreview'),
            bs.Level(name='Pro nuclear mad',
                     displayName='Pro ${GAME}',
                     gameType=TNTMadGame,
                     settings={'preset':'pro'},
                     previewTexName='courtyardPreview')]

class TNTMadGame(bs.TeamGameActivity):
    ...
    def onBegin(self):
        ...
        bs.gameTimer(1000, bs.Call(self._bots.spawnBot, bot.BomberTNTBot,
                                   pos=(3, 3, -2), spawnTime=3000))
        bs.gameTimer(2000, bs.Call(self._bots.spawnBot, bot.BomberTNTBot,
                                   pos=(-3, 3, -2), spawnTime=3000))
        bs.gameTimer(3000, bs.Call(self._bots.spawnBot, bot.BomberTNTBot,
                                   pos=(5, 3, -2), spawnTime=3000))
        bs.gameTimer(4000, bs.Call(self._bots.spawnBot, bot.BomberTNTBot,
                                   pos=(-5, 3, -2), spawnTime=3000))
            
        # add some extras for multiplayer or pro mode
        if len(self.initialPlayerInfo) > 2 or isPro:
            bs.gameTimer(5000, bs.Call(self._bots.spawnBot, bot.BomberTNTBot,
                                       pos=(0, 3, -5), spawnTime=3000))
        if len(self.initialPlayerInfo) > 3 or isPro:
            bs.gameTimer(6000, bs.Call(self._bots.spawnBot, bot.BomberTNTBot,

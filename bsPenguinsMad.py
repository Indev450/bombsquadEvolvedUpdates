...
def bsGetLevels():
    ...
    return [bs.Level(name='Penguin Fight',  # (unique id not seen by player)
                     displayName='${GAME}',  # (readable name seen by player)
                     gameType=PenguinFightGame,
                     settings={'preset':'regular'},
                     previewTexName='hockeyStadiumPreview'),
            bs.Level(name='Pro penguin fight',
                     displayName='Pro ${GAME}',
                     gameType=PenguinFightGame,
                     settings={'preset':'pro'},
                     previewTexName='hockeyStadiumPreview')]

class PenguinFightGame(bs.TeamGameActivity):
    ...
    def getDescription(cls, sessionType):
        return 'How fast can you defeat the penguins?'
    ...
    def onBegin(self):
        ...
        bs.gameTimer(1000, bs.Call(self._bots.spawnBot, bot.PenguinBot,
                                   pos=(3, 3, -2), spawnTime=3000))
        bs.gameTimer(2000, bs.Call(self._bots.spawnBot,
                                   bot.PenguinBot, pos=(-3, 3, -2),
                                   spawnTime=3000))
        bs.gameTimer(3000, bs.Call(self._bots.spawnBot, bot.PenguinBot,
                                   pos=(5, 3, -2), spawnTime=3000))
        bs.gameTimer(4000, bs.Call(self._bots.spawnBot, bot.PenguinBot,
                                   pos=(-5, 3, -2), spawnTime=3000))
            
        # add some extras for multiplayer or pro mode
        if len(self.initialPlayerInfo) > 2 or isPro:
            bs.gameTimer(5000, bs.Call(self._bots.spawnBot, bot.PenguinBot,
                                       pos=(0, 3, -5), spawnTime=3000))
        if len(self.initialPlayerInfo) > 3 or isPro:
            bs.gameTimer(6000, bs.Call(self._bots.spawnBot, bot.PenguinBot,
